...
$qtable="SELECT *FROM hod WHERE  username='$uname'";
	   ...
	   if((empty($uname))||(empty($pswd)))
		   {
		    $error.="<p style=\"color:#e79258\">Fill all the fields </p>"; 	
				...
				{
				  if($row['deptID']==1){
				  	header('location:http://localhost/COLS1/pages/information.php');
					$_SESSION['deptID']=$row['deptID'];
				  }elseif($row['deptID']==2){
				  header('location:http://localhost/COLS1/pages/finance.php');
				  $_SESSION['deptID']=$row['deptID'];
				  }elseif($row['deptID']==3){
				  	header('location:http://localhost/COLS1/pages/research.php');
					$_SESSION['deptID']=$row['deptID'];
				  }else{
				  	header('location:http://localhost/COLS1/pages/audit.php');
					$_SESSION['deptID']=$row['deptID'];
				  }
...
<div id="templatemo_main">
 ...
 ?>
  <p style="color:#d2d74b;margin-left:-50px;">HOD login page</p>
  <h3>LOGIN</h3>
